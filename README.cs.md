...
### Odinstalace, cleanup a přeinstalace

**Odstranění rozšíření (běžný uninstall):**

1. **Panel Extensions:** najdi *Cursor plan usage (status bar)* → **Uninstall**.
2. **Nebo CLI** (pokud máš `cursor` v PATH):

   ```powershell
   cursor --uninstall-extension lukasvladyka.cursor-usage-status
   ```

   Pokud uvidíš *Extension 'lukasvladyka.cursor-usage-status' was successfully uninstalled!* — hotovo. Reload Window je volitelný; Cursor změnu často načte hned.

**Přeinstalace VSIX (upgrade nebo čistá instalace):**

1. Nejdřív odinstaluj aktuální kopii (kroky výše).
2. Znovu nainstaluj **Možností A** (Command Palette → *Extensions: Install from VSIX...*).
3. Pokud se položka ve status baru neobjeví, spusť **Developer: Reload Window**.

> **VSIX soubor vs složka na disku:** stahuješ `cursor-usage-status-*.vsix`, ale Cursor instaluje do `{publisher}.{název}-{verze}` z [`package.json`](./cursor-usage-status/package.json) — ne podle názvu `.vsix`.

**Starší publisher `local` (jen pokud jsi dřív kopíroval soubory ručně):**

Rané ruční instalace někdy používaly publisher **`local`** místo **`lukasvladyka`**. Pokud jsi to kdysi dělal, spusť navíc:

```powershell
cursor --uninstall-extension local.cursor-usage-status
```

Když CLI odpoví *Extension 'local.cursor-usage-status' is not installed* — **to je v pořádku**. Znamená to, že starou kopii nemáš; stačí `lukasvladyka.cursor-usage-status`.

### Instalace — řešení problémů

**Chyba: *„Please restart VS Code before reinstalling Cursor plan usage (status bar).“***

Samotný restart Cursoru **tento stav neopraví**. V registru rozšíření (`extensions.json`) zůstane **ghost záznam**, ale složka na disku chybí — např. po ručním smazání složky, přerušené VSIX instalaci nebo přechodu ze staré kopie `local.*` na VSIX.

**Oprava:**

1. **CLI uninstall** (zkus obě ID; u `local.*` ignoruj *not installed*, pokud jsi ho nikdy neměl):
...
2. **Pokud instalace pořád selže — reset skript** z repa (spouštěj jen z oficiálního klonu; Cursor úplně zavřený):
...
3. **Poslední možnost — ruční editace JSON:** zálohuj `%USERPROFILE%\.cursor\extensions\extensions.json` (macOS/Linux: `~/.cursor/extensions/extensions.json`), odstraň záznamy s `identifier.id` `lukasvladyka.cursor-usage-status` a/nebo `local.cursor-usage-status`. **Nikdy needituj za běhu Cursoru.**

4. Znovu otevři Cursor → **Extensions: Install from VSIX...** → vyber `.vsix`. **Developer: Reload Window** jen pokud status bar chybí.

**Nikdy nemaž ručně složky** z `.cursor\extensions\` — vždy **Uninstall** v panelu Extensions nebo CLI příkazy výše.
